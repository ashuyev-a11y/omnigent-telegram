...
1. Скопируйте пример конфига и отредактируйте под себя:
   ...
   `config.yaml` в `.gitignore` — не коммитьте его, в нём реальные пути и id.
...
Нет approval-карточек и кнопок, нет продолжения диалога с сессией, нет
параллельных задач в одном чате, нет доступа к Supabase/Vercel/ключам
проектов, нет автоматического определения проекта по тексту, нет вебхуков
(только long polling).
